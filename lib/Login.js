	...
	if( username === "Guest" && password === undefined ) {
		...
		_db.collection('users').findAndModify({
			...
			if(err) {
				...
				debug("Guest logged in %j", result);
				_id=result._id;
				debug("resolving login promise");
				login_promise.resolve(result.value);
			}
		});
	} else {
...
function who(client) {
	_db.collection("users").find({logged_in:true}, function whoResults(err, results) {
		if(err) {
			...
			results.toArray(function arrayOfResults(err, a) {
				debug("printing users %j", results);
				async.each( a, function eachLoggedInUser(user, finished) {
...
function quit(client) {
	...
	if( _id ) {
		_db.update({_id:_id}, {"$set":{logged_in:false}}, function(err) {
			if(err) debug("error logging out %j", err);
			log("db updated");
			_id=undefined;
			client.end();
		});
