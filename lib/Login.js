...
function login(username, password) {
	...
	if( username === "Guest" && password === undefined ) {
		...
		// look up username in db
	 	_db.collection("users").findAndModify({ email_address: username, password: password }, { $set:{ logged_in:true } }, function login_hollaback(err, result) {
	 		if(err) {
	 			login_promise.reject(err);
		 	}
	 		if( result.length === 0 ) {
	 			login_promise.reject("email/password combination doesn't match");
	 		} else {
	 			login_promise.resolve(result);
	 		}
	 	});
	 }
	 debug("returning promise %j", login_promise);
